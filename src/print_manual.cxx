...
void unvm::PrintManual()
{
    std::cerr
            ...
            << "  list,             l [-a|--available] [-f|--flat] [-d|--details] [-r|--tracks]  List installed versions. Use `-a` or `--available` to list version available online. Use `-f` or `--flat` to print as a flat list. Use `-d` or `--details` to print more details and subversions. Use `-t` or `--track` to list only tracked versions.\n"
